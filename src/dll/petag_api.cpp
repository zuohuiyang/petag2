...
uint32_t WINAPI InsertPeTag(const wchar_t* filePath,
  ...
  std::string err;
  std::vector<uint8_t> bytes = petag::ReadFileBytes(filePath, &err);
  if (bytes.empty()) {
    return PETAG_E_IO;
  }
  if (!petag::IsPEFile(bytes)) {
    return PETAG_E_FORMAT;
  }

  std::string ascii(tagData, tagData + static_cast<size_t>(tagLen));
  std::vector<uint8_t> encoded = petag::EncodeMetadata(ascii, &err);
  if (encoded.empty()) {
    return PETAG_E_INVALID_ARG;
  }
  ...
  bool overwrite = wcscmp(filePath, outputFilePath) == 0 ||
                   petag::FileExists(outputFilePath);
  if (!petag::WriteFileBytes(outputFilePath, *updated, overwrite, &err)) {
    return PETAG_E_IO;
  }
  return PETAG_OK;
}
...
uint32_t WINAPI ReadPeTag(const wchar_t* filePath,
  ...
  *outLen = 0;
  ...
  std::string meta;
  if (!petag::DecodeMetadata(*t, &meta, &err)) {
    return PETAG_E_CERT;
  }
